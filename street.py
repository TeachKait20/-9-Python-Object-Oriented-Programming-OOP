...
# Статический метод (@staticmethod) — не зависит ни от экземпляра класса, ни от самого класса. Используется, когда метод работает только с входными данными и не использует атрибуты класса или экземпляра.
# Метод класса (@classmethod) — работает с самим классом и имеет доступ к его атрибутам через аргумент cls.
